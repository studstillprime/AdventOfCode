        ...
        for (int boardNumber = 0; boardNumber < 100; boardNumber++)
        {
            for (int spaceNumber = 0; spaceNumber < 25; spaceNumber++)
            {
                if (ball == boards[boardNumber][spaceNumber])
                {
                    ...
                    printf("MATCH FOUND FOR\nBALL %i: %i\nBOARD: %i\nSPACE: %i\nMATCH NUMBER: %i\n", ballNumber, ball, boardNumber, spaceNumber, matchCount);
...
int checkBingo(int board, int space)
{
    ...
    switch(space)
    {
        ...
        case 18 :
        case 24 :
            if (boards[board][0] == 100 && boards[board][6] == 100 && boards[board][12] == 100 && boards[board][18] == 100 && boards[board][24] == 100)
                {
                    printf("0 - 24 Bingo On Board: %i\n", board);
                    return 1;
                }
        default :
         printf("No 0-24 Bingo\n" );
   }
    //4 - 20 Bingo
    switch(space)
    {
        ...
        case 16 :
        case 20 :
            if (boards[board][4] == 100 && boards[board][8] == 100 && boards[board][12] == 100 && boards[board][16] == 100 && boards[board][20] == 100)
                {
                    printf("4 - 20 Bingo On Board: %i\n", board);
                    return 1;
                }
        default :
         printf("No 4-20 Bingo\n" );
   }
   return 0;
}